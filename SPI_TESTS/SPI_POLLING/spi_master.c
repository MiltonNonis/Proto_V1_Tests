...
#include "collar_defines.h" // This file must be in the application folder
...
uint32_t* spi_master_init(SPIModuleNumber module_number, SPIMode mode, bool lsb_first)
{
    ...
    NRF_SPI_Type *spi_base_address = (SPI_MASTER_0 == module_number)? NRF_SPI0 : (NRF_SPI_Type *)NRF_SPI1;

    if(SPI_MASTER_0 == module_number)
    {
#if defined(ACCELEROMETER_ENABLE)
        /* Configure GPIO pins used for pselsck, pselmosi, pselmiso and pselss for SPI0 */
        nrf_gpio_cfg_output(ACCEL_SPI_SCK_PIN);
        nrf_gpio_cfg_output(ACCEL_SPI_MOSI_PIN);
        nrf_gpio_cfg_input(ACCEL_SPI_MISO_PIN, NRF_GPIO_PIN_NOPULL);
        nrf_gpio_cfg_output(ACCEL_SPI_SS_PIN);

        /* Configure pins, frequency and mode */
        spi_base_address->PSELSCK  = ACCEL_SPI_SCK_PIN;
        spi_base_address->PSELMOSI = ACCEL_SPI_MOSI_PIN;
        spi_base_address->PSELMISO = ACCEL_SPI_MISO_PIN;
        nrf_gpio_pin_set(ACCEL_SPI_SS_PIN); /* disable Set slave select (inactive high) */
#else
#endif        
    }
    else
    ...
    spi_base_address->FREQUENCY = (uint32_t) SPI_FREQUENCY_FREQUENCY_M1;    // SPI_OPERATING_FREQUENCY;
...
bool spi_master_tx_rx(uint32_t *spi_base_address, uint16_t transfer_size, const uint8_t *tx_data, uint8_t *rx_data)
{
    ...
    {
#if defined(ACCELEROMETER_ENABLE)
        SEL_SS_PINOUT = ACCEL_SPI_SS_PIN;
#else
